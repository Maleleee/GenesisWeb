...
data_path = os.environ.get('DATA_PATH', "attack_data_updated.csv")
data = pd.read_csv(data_path)

# Ensure the DataFrame has the correct columns
data.columns = ['timestamp', 'packet_size', 'source_ip', 'destination_ip', 'request_rate', 'protocol', 'tcp_port', 'udp_port', 'label']
...
def fetch_attack_data():
    # Load the new CSV file with the updated structure
    data = pd.read_csv('attack_data_updated.csv')
    ...
    for index, row in data.iterrows():
        processed_data.append({
            ...
            'tcp_port': row['tcp.dstport'],
            'udp_port': row['udp.dstport'],
            'label': row['label']
        })
...
@app.route('/get_attack_data', methods=['GET'])
def get_attack_data():
    attack_data = fetch_attack_data()  # Fetch the updated data
    return jsonify(attack_data)  # Return the data as JSON
...
if __name__ == '__main__':
    ...
    # model_thread = Thread(target=simulate_attack, args=(3600,))
